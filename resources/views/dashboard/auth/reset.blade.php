                        ...
                        <div class="card mb-0">
                            <div class="card-body">
                                <a href="{{ route('site.home') }}" class="brand-logo">
                                    <img src="{{ asset('storage/' . getSetting('logo')) }}" width="200px"
                                        alt="">
                                </a>

                                <h4 class="card-title mb-1">{{ __('models.forget_pass') }} 🔒</h4>
                                {{-- <p class="card-text mb-2">أدخل بريدك الإلكتروني وسنرسل لك تعليمات لإعادة تعيين كلمة مرورك</p> --}}

                                <form class="auth-forgot-password-form mt-2" action="{{ route('admin.sendLink') }}"
                                    method="POST">
                                    @csrf()
                                    <div class="form-group">
                                        <label for="forgot-password-email"
                                            class="form-label">{{ __('models.email') }}</label>
                                    ...
                                    <button class="btn btn-primary btn-block"
                                        tabindex="2">{{ __('models.send_reset_link') }}</button>
                                </form>

                                <p class="text-center mt-2">
                                    <a href="{{ route('admin.login') }}"> <i data-feather="chevron-left"></i>
                                        {{ __('models.login') }} </a>
                                </p>
                            </div>
                        </div>
